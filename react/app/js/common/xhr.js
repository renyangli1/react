...
module.exports = function(options) {
    ...
    if(options.authRequired && (cache.getSessionId() || '') < 8){
        (options.sessionExpired || sessionExpiredDialog)({c:-3});
        return;
    }
    ...
    url = (!new RegExp('^http', 'i').test(url) && $.XMCfg.biz || '') + url + '&g=' + g;

    var opts = {
            type: 'POST', //hard code, we only support post for encryption purpose
            url: url,
            contentType: typeof options.contentType === 'undefined' ? 't/j' : options.contentType,
            ...
            success: function(res) {
                ...
                try {
                    if(!res.c) {
                        ...
                        if(res.c === -3) {
                            var loggedIn = (cache.getSessionId() || '').length >= 8;
                            cache.removeItem(cache.CONST.SESSION_ID);
                            cache.removeItem(cache.CONST.UID);
                            if(loggedIn) { //been kicked off, for security reason, warn user to login again
                                window.WEB.route.removeTabCache();
                                window.WEB.route.forward('tab/login/security');
                            } else {
                                (options.sessionExpired || sessionExpiredDialog)(res);
                            }

                        } else if(res.c === 'update') {
